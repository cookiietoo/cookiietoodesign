...
- `lib/content/site.js` — identity, `skills`, `accentSkills`, `clients`, links, `cvPath`, `status`. **`role` is intentionally the neutral "Designer" (not "Product Designer"/"UX"/"Senior") so the headline doesn't conflict with UX / UX-UI job applications**, paired with an `experience` field ("7 Years Experience"). The hero renders `{site.role} · {site.experience}`; `IntroStatement` also uses `site.role`. Keep "Senior"/"Sr." and discipline-specific labels out of headline copy.
...
- `TorchHero` is the signature piece: desktop reveals scattered skill words through a cursor-following radial mask; touch/reduced-motion renders the skills as a wrapped chip cloud instead. The reveal size is the `circle 200px` radius in the radial-gradient mask (three occurrences in `TorchHero.jsx` — the live mousemove handler and the initial off-screen mask; keep them in sync).
